"""
Generate a TikZ scatterplot: number of candidate commits (x) vs number of tool calls (y).

Produces:
  - figures/scatter_toolcalls_vs_candidates.tex  (inputable in a paper)
  - figures/scatter_toolcalls_vs_candidates_standalone.tex  (compilable with pdflatex)
"""
...
def load_data(results_file):
    ...
    for d in data["details"]:
        ...
        if cs is None or candidates is None:
            continue
        tool_calls = cs.get("total_tool_calls")
        if tool_calls is not None:
            points.append((candidates, tool_calls))
    return points
...
def generate_tikz_body(points):
    ...
    y_ceil = int(math.ceil(y_max / 5) * 5)
    ...
    lines.append(r"    ylabel={Number of tool calls},")
    lines.append(r"    xmode=log,")
    lines.append(f"    ymin=0, ymax={y_ceil},")
    ...
    for candidates, tool_calls in points:
        lines.append(f"    ({candidates}, {tool_calls})")
...
def write_inputable(tikz_body):
    path = os.path.join(OUTPUT_DIR, "scatter_toolcalls_vs_candidates.tex")
...
def write_standalone(tikz_body):
    path = os.path.join(OUTPUT_DIR, "scatter_toolcalls_vs_candidates_standalone.tex")
