#!/usr/bin/env python3
"""Baseline AG-SZZ evaluation script.

This script runs AG-SZZ from pyszz_v2 on a sampled dataset and evaluates the results.

Usage:
    python baseline_agszz.py
    python baseline_agszz.py --limit 10  # Process only 10 samples
    python baseline_agszz.py --sample-file sampled_datasets/DS_GITHUB-j.json
"""
...
# Import pyszz modules (skip the requirements check)
...
def run_agszz_on_entry(
    entry: Dict,
    repos_dir: Path,
    config: Dict
) -> List[str]:
    """Run AG-SZZ on a single entry and return predicted BICs."""
...
def main():
    parser = argparse.ArgumentParser(description="Run AG-SZZ baseline evaluation")
    parser.add_argument(
        "--limit", "-l",
        type=int,
        default=None,
    ...
    # Resolve sample file path before changing directory
    sample_file = Path(args.sample_file).resolve()
    # Change to project root directory
    os.chdir(Path(__file__).parent.parent)

    print("=" * 70)
    print("              AG-SZZ Baseline Evaluation")
    print("=" * 70)
    ...
    for idx, entry in enumerate(sample_data, 1):
        ...
        predicted_bics = run_agszz_on_entry(entry, REPOS_DIR, AGSZZ_CONFIG)
    ...
    # Print summary
    print_summary(summary, "AG-SZZ")

    # Save detailed results
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = RESULTS_DIR / f"baseline_agszz_{timestamp}.json"
