#!/usr/bin/env python3
"""Baseline MA-SZZ evaluation script.

This script runs MA-SZZ from pyszz_v2 on a sampled dataset and evaluates the results.

Usage:
    python baseline_maszz.py
    python baseline_maszz.py --limit 10  # Process only 10 samples
    python baseline_maszz.py --sample-file sampled_datasets/DS_GITHUB-j.json
"""
...
# MA-SZZ config (from conf/maszz.yml)
MASZZ_CONFIG = {
    "szz_name": "ma",
...
def run_maszz_on_entry(
    entry: Dict,
    repos_dir: Path,
    config: Dict
) -> List[str]:
    """Run MA-SZZ on a single entry and return predicted BICs."""
...
def main():
    parser = argparse.ArgumentParser(description="Run MA-SZZ baseline evaluation")
    parser.add_argument(
        "--limit", "-l",
        type=int,
        default=None,
    ...
    print("=" * 70)
    print("              MA-SZZ Baseline Evaluation")
    print("=" * 70)
    ...
    for idx, entry in enumerate(sample_data, 1):
        ...
        predicted_bics = run_maszz_on_entry(entry, REPOS_DIR, MASZZ_CONFIG)
    ...
    print_summary(summary, "MA-SZZ")

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = RESULTS_DIR / f"baseline_maszz_{timestamp}.json"
