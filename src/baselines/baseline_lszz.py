#!/usr/bin/env python3
"""Baseline L-SZZ evaluation script.

This script runs L-SZZ from pyszz_v2 on a sampled dataset and evaluates the results.

Usage:
    python baseline_lszz.py
    python baseline_lszz.py --limit 10  # Process only 10 samples
    python baseline_lszz.py --sample-file sampled_datasets/DS_GITHUB-j.json
"""
...
# L-SZZ config (from conf/lszz.yml)
LSZZ_CONFIG = {
    "szz_name": "l",
...
def run_lszz_on_entry(
    entry: Dict,
    repos_dir: Path,
    config: Dict
) -> List[str]:
    """Run L-SZZ on a single entry and return predicted BICs."""
    ...
    try:
        ...
        bug_inducing_commits = l_szz.find_bic(
            fix_commit_hash=fix_commit,
...
def main():
    parser = argparse.ArgumentParser(description="Run L-SZZ baseline evaluation")
    parser.add_argument(
        "--limit", "-l",
        type=int,
        default=None,
    ...
    print("=" * 70)
    print("              L-SZZ Baseline Evaluation")
    print("=" * 70)
    ...
    for idx, entry in enumerate(sample_data, 1):
        ...
        predicted_bics = run_lszz_on_entry(entry, REPOS_DIR, LSZZ_CONFIG)
    ...
    print_summary(summary, "L-SZZ")

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = RESULTS_DIR / f"baseline_lszz_{timestamp}.json"
