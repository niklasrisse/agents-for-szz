#!/usr/bin/env python3
"""Baseline RA-SZZ evaluation script.

This script runs RA-SZZ from pyszz_v2 on a sampled dataset and evaluates the results.

NOTE: RA-SZZ uses RefactoringMiner which only works with Java files. For non-Java
repositories (like Linux kernel), it will behave similarly to MA-SZZ without
refactoring detection.

Usage:
    python baseline_raszz.py
    python baseline_raszz.py --limit 10  # Process only 10 samples
    python baseline_raszz.py --sample-file sampled_datasets/DS_GITHUB-j.json
"""
...
# RA-SZZ config (from conf/raszz.yml)
# Note: Original config limits to .java files, but we remove that restriction
# to allow analysis of C files (though refactoring detection won't work for C)
RASZZ_CONFIG = {
    "szz_name": "ra",
    ...
    # "file_ext_to_parse": ["java"],  # Removed to allow C files
}
...
def run_raszz_on_entry(
    entry: Dict,
    repos_dir: Path,
    config: Dict
) -> List[str]:
    """Run RA-SZZ on a single entry and return predicted BICs."""
    ...
    try:
        ...
        bug_inducing_commits = ra_szz.find_bic(
            fix_commit_hash=fix_commit,
            impacted_files=imp_files,
            max_change_size=config.get('max_change_size', 20),
            detect_move_from_other_files=DetectLineMoved(config.get('detect_move_from_other_files', 1)),
...
def main():
    parser = argparse.ArgumentParser(description="Run RA-SZZ baseline evaluation")
    parser.add_argument(
        "--limit", "-l",
        type=int,
        default=None,
    ...
    print("=" * 70)
    print("              RA-SZZ Baseline Evaluation")
    print("=" * 70)
    print("\nNote: RA-SZZ uses RefactoringMiner which only detects Java refactorings.")
    print("For non-Java code, it behaves similarly to MA-SZZ.")
    ...
    for idx, entry in enumerate(sample_data, 1):
        ...
        predicted_bics = run_raszz_on_entry(entry, REPOS_DIR, RASZZ_CONFIG)
    ...
    print_summary(summary, "RA-SZZ")

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = RESULTS_DIR / f"baseline_raszz_{timestamp}.json"
