#!/usr/bin/env python3
"""Baseline R-SZZ evaluation script.

This script runs R-SZZ from pyszz_v2 on a sampled dataset and evaluates the results.

Usage:
    python baseline_rszz.py
    python baseline_rszz.py --limit 10  # Process only 10 samples
    python baseline_rszz.py --sample-file sampled_datasets/DS_GITHUB-j.json
"""
...
# R-SZZ config (from conf/rszz.yml)
RSZZ_CONFIG = {
    "szz_name": "r",
...
def run_rszz_on_entry(
    entry: Dict,
    repos_dir: Path,
    config: Dict
) -> List[str]:
    """Run R-SZZ on a single entry and return predicted BICs."""
    ...
    try:
        ...
        bug_inducing_commits = r_szz.find_bic(
            fix_commit_hash=fix_commit,
...
def main():
    parser = argparse.ArgumentParser(description="Run R-SZZ baseline evaluation")
    parser.add_argument(
        "--limit", "-l",
        type=int,
        default=None,
    ...
    print("=" * 70)
    print("              R-SZZ Baseline Evaluation")
    print("=" * 70)
    ...
    for idx, entry in enumerate(sample_data, 1):
        ...
        predicted_bics = run_rszz_on_entry(entry, REPOS_DIR, RSZZ_CONFIG)
    ...
    print_summary(summary, "R-SZZ")

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = RESULTS_DIR / f"baseline_rszz_{timestamp}.json"
